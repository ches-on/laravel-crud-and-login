...
protected function explodeExplicitRule($rule, $attribute)
{
    ...
    if (is_int($rule)) {
        // Handle the case where $rule is an integer
        return [$rule];
    }

    if (!is_array($rule)) {
        // Handle the case where $rule is not an array, string, object, or integer
        throw new \InvalidArgumentException('Expected an array, string, object, or integer, but got: ' . gettype($rule));
    }
